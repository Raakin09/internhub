...
import {
  HiOutlineLocationMarker, HiOutlineCurrencyRupee,
  HiOutlineOfficeBuilding, HiOutlineUserGroup,
  HiOutlineArrowLeft, HiOutlineCheckCircle, HiOutlineBriefcase,
} from 'react-icons/hi';
  ...
  if (!job) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-lg font-medium text-dark-200">Job not found</p>
        <Link href="/jobs" className="text-sm text-primary-light hover:text-primary">{t('common.back')}</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen pb-12">
      <div className="bg-dark-950/50 border-b border-white/5">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6">
          <Link href="/jobs" className="inline-flex items-center gap-2 text-sm text-dark-400 hover:text-dark-200 transition-colors mb-6">
            <HiOutlineArrowLeft className="w-4 h-4" /> {t('common.back')} to Jobs
          </Link>
        </div>
      </div>
      <div className="max-w-4xl mx-auto px-4 sm:px-6 pt-8">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-3xl p-8">
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8">
            <div>
              <h1 className="text-2xl font-bold text-dark-50 mb-2">{job.title}</h1>
              <p className="text-dark-400 flex items-center gap-2">
                <HiOutlineOfficeBuilding className="w-4 h-4" /> {job.company}
              </p>
            </div>
            <div>
              {applied ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div className="p-4 rounded-xl bg-dark-800/50 border border-white/5">
              ...
              <p className="text-sm font-semibold text-dark-100">{job.location}</p>
            </div>
            <div className="p-4 rounded-xl bg-dark-800/50 border border-white/5">
              <div className="flex items-center gap-2 text-xs text-dark-500 mb-1"><HiOutlineCurrencyRupee className="w-4 h-4" /> Salary</div>
              <p className="text-sm font-semibold text-dark-100">{formatSalary(job.salaryMin, job.salaryMax)}</p>
            </div>
            <div className="p-4 rounded-xl bg-dark-800/50 border border-white/5">
              <div className="flex items-center gap-2 text-xs text-dark-500 mb-1"><HiOutlineBriefcase className="w-4 h-4" /> Experience</div>
              <p className="text-sm font-semibold text-dark-100 capitalize">{job.experienceLevel || 'Not specified'}</p>
            </div>
            <div className="p-4 rounded-xl bg-dark-800/50 border border-white/5">
              <div className="flex items-center gap-2 text-xs text-dark-500 mb-1"><HiOutlineUserGroup className="w-4 h-4" /> Applicants</div>
              <p className="text-sm font-semibold text-dark-100">{job.applicantCount || 0}</p>
            </div>
          </div>
          {job.workType && (
            <div className="mb-6">
              <span className={`inline-block px-3 py-1 rounded-lg text-xs font-semibold border ${workTypeColors[job.workType] || ''}`}>
          ...
          {job.description && (
            <div className="mb-8">
              <h3 className="text-sm font-semibold text-dark-200 mb-3">About the Role</h3>
              <p className="text-sm text-dark-400 leading-relaxed whitespace-pre-wrap">{job.description}</p>
            </div>
          )}
        </motion.div>
      </div>
